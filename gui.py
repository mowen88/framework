...
class GUI:
	def __init__(self, game, level):
		...
		# minimap image
		self.minimap_image = pygame.image.load(f'assets/tracks/{self.level.track}/minimap.png').convert_alpha()
		...
		self.race_over_menu = RaceOver(self.game, self.level.car_type, self.level.track)
	...
	def update(self):
		...
		self.render_text("Prev Lap", WHITE, self.game.smaller_font, (WIDTH * 0.05, HEIGHT * 0.05), False)
		self.render_text("Best Lap", BLACK, self.game.smaller_font, (WIDTH * 0.05, HEIGHT * 0.18), False)

		# render actual values for best and prev lap
		if not self.level.lap_times:
			self.render_text("-- : -- : ---", WHITE, self.game.smaller_font, (WIDTH * 0.05, HEIGHT * 0.1), False)
			self.render_text("-- : -- : ---", BLACK, self.game.smaller_font, (WIDTH * 0.05, HEIGHT * 0.23), False)
		else:
			self.render_text(f"{self.level.lap_times[-1]}", WHITE, self.game.smaller_font, (WIDTH * 0.05, HEIGHT * 0.1), False)
			self.render_text(f"{min(self.level.lap_times)}", BLACK, self.game.smaller_font, (WIDTH * 0.05, HEIGHT * 0.23), False)
		...
		if len(self.level.lap_times) >= self.level.total_laps:
			self.render_text(f"Lap {self.level.total_laps} of {self.level.total_laps}", WHITE, self.game.small_font, (((WIDTH * 0.05) - (2 * SCALE)), HEIGHT * 0.3), False)
			...
			self.render_text(f"Lap {len(self.level.lap_times)+1} of {self.level.total_laps}", WHITE, self.game.small_font, (((WIDTH * 0.05) - (2 * SCALE)), HEIGHT * 0.3), False)
